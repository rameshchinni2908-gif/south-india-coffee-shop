...
`GET /api/admin/agent/status` returns whether the assistant is configured without calling OpenAI. An answer is generated only after an admin submits a question. Each production run makes at most three model requests across two tool rounds, with at most one call each to the menu and report tools and at most three change proposals. A question answered fully by reference notes needs no live read. There are no automatic retries or background answers. Only one answer runs at a time in this API process. These limits are held in memory and reset on restart; additional API instances would need a shared limiter. API credits are still required for each live model request.
