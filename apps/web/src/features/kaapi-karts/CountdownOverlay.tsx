...
import { GAME_TAGLINE } from "./game-contract.js";

const TICK_MS = 80;
/** How long "GO" stays on screen after lights-out. */
...
interface CountdownOverlayProps {
  /** Lights-out in *local* clock time (server timestamp + clock offset). */
  ...
  return (
    <Box
      sx={{
        ...
        color: "#fffdf8",
        background:
          "radial-gradient(120% 90% at 50% 30%, rgba(111, 50, 25, 0.94), rgba(30, 15, 9, 0.97))",
        paddingTop: "env(safe-area-inset-top)",
        paddingBottom: "env(safe-area-inset-bottom)",
        "@keyframes kaapiCountdownPop": {
      ...
      <Box>
        ...
        <Typography
          ...
          sx={{
            ...
            animation: reducedMotion ? "none" : "kaapiCountdownPop 900ms ease-out both",
          }}
        >
          {label}
        </Typography>
        <Box aria-live="assertive" aria-atomic sx={{ mt: 1 }}>
          <Typography sx={{ opacity: 0.85, fontWeight: 700 }}>
            {isGo ? "Go! Three laps." : `Starting in ${label}`}
          </Typography>
        </Box>
