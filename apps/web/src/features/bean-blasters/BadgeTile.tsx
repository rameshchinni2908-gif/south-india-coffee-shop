...
import { findBadgeColour, formatBadgeNumber, type ArenaPlayer } from "./arena-contract.js";

const FALLBACK_COLOUR = "#6f3219";
...
export const BadgeTile = ({ player, isYou, reducedMotion }: BadgeTileProps) => {
  const colour = findBadgeColour(player.colour);
  const colourHex = colour?.hex ?? FALLBACK_COLOUR;
  const colourLabel = colour?.label ?? "Unassigned colour";
  const number = formatBadgeNumber(player.badgeNumber);
  ...
  return (
    <Paper
      component="li"
      variant="outlined"
      aria-label={`Badge ${number}, ${colourLabel}, ${readyLabel}${isYou ? ", you" : ""}${
        player.isHost ? ", host" : ""
      }, ${connectionLabel}`}
      sx={{
        position: "relative",
